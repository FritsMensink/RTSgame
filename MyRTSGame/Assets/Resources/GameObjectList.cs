...
public class GameObjectList : MonoBehaviour {
	...
	void Awake() {
		if(!created) {
			//als we een nieuwe map laden moet dit object blijven bestaan
			DontDestroyOnLoad(transform.gameObject);
			ResourceManager.SetGameObjectList(this);
			//om aan te geven dat er al 1 is gemaakt.
			created = true;
		} else {
			//als er toch nog een nieuwe gameobjectlist wordt aangemaakt verwijder die dan.
